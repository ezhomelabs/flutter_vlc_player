...
void
Java_org_videolan_libvlc_Media_nativeNewFromHaicamCallbacks(
    JNIEnv *env, jobject thiz, jobject libVlc, jint type, jlong accountId, jlong cameraId)
{
    vlcjni_object *p_obj;

    if(
        haicam_libvlc_play_callback == NULL
        || haicam_libvlc_open_callback == NULL
        || haicam_libvlc_read_callback == NULL
        || haicam_libvlc_seek_callback == NULL
        || haicam_libvlc_close_callback == NULL
    ) {
        throw_Exception(env, VLCJNI_EX_ILLEGAL_ARGUMENT, "Haicam libvlc callbacks are not set");
        return;
    }
    ...
    p_obj->data = haicam_libvlc_play_callback(type, accountId, cameraId);

    p_obj->u.p_m =
        libvlc_media_new_callbacks(p_obj->p_libvlc,
                                   haicam_libvlc_open_callback,
                                   haicam_libvlc_read_callback,
                                   haicam_libvlc_seek_callback,
                                   haicam_libvlc_close_callback,
                                   p_obj);
